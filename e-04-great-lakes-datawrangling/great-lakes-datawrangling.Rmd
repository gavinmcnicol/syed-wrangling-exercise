...
catch_data_us %>%
  ...
  summarise(stats = mean(values, na.rm = TRUE),
            median = median(values, na.rm = TRUE),
            minimun  = min(values,na.rm =TRUE),
...
```{r sums-mussel species- michigan}
catch_data_us %>%
  filter(lake == "Michigan",
         year %in% c(1975,1985,1995,2005,2015)) %>%   group_by(year) %>%
...
catch_data_us %>%
  ...
  mutate(annual_catch_lsb = total_fish)
